    ...
    try {
        const total = await page.evaluate(() => {
            const m = document.querySelectorAll(
                "#main > div.org-grid__content-height-enforcer > div > div.artdeco-card.pv5.pl5.pr1.mt4 > div > div.scaffold-finite-scroll__content > ul > li "
            ).length;
...
export async function getCompaniesPageUrl() {
    ...
    try {
        await page.goto(
            "https://www.welcometothejungle.com/fr/jobs?groupBy=job&sortBy=mostRecent&query=&aroundQuery=France%2C+France&refinementList%5Boffice.country_code%5D%5B%5D=FR&refinementList%5Bprofession_name.fr.Tech%5D=&page=1"
        );
        await promise;
    } catch (e) {
        console.log(e);
    ...
    try {
        const links = await page.evaluate(() => {
            const total = [];
            linkslist = document.querySelectorAll(
                "li.ais-Hits-list-item article div:nth-child(1) a"
            );
        ...
        const total = await page.evaluate(() => {
            if (
                document.querySelector(
                    "div.g > div > div > div > div:nth-child(1) > div > a > h3"
                )?.textContent
            ) {
                ...
                document.querySelector(
                    "div.g > div > div:nth-child(1) > div > a > h3"
                )?.textContent
            ) {
                return document.querySelector(
                    "div.g > div > div:nth-child(1) > div > a > h3"
    ...
    for (let i = 0; i < links.length; i++) {
        try {
            ...
            let total = await page.evaluate(() => {
                ...
                for (e of linksList) {
                    if (
                        e.querySelector("a i").getAttribute("name") ===
                        "linkedin"
                    ) {
...
async function linkedinScraper(browser, page, cookie, linkedinUrls) {
    ...
    try {
        ...
        for (let i = 0; i < linkedinUrls.length; i++) {
            ...
            let title = "";
            try {
                title = await page.$eval("div h1 span", (el) =>
                    el.textContent.trim()
                );
            } catch (e) {
                console.log(e);
            ...
            const employeesBio = await page.evaluate(async () => {
                ...
                try {
                    const lists = document.querySelectorAll(
                        "#main > div.org-grid__content-height-enforcer > div > div.artdeco-card.pv5.pl5.pr1.mt4 > div > div.scaffold-finite-scroll__content > ul > li >section > div > div > div:nth-child(2)"
                    );
                    const dictionary =
                        /founder|fondateur|co-founder|co-fondateur|co founder|co fondateur|director|directeur|ceo|cto|chief \w+ officer/i;
                    for (let e of lists) {
                        const bio = e
            ...
            for (let i = 0; i < employeesBio.length; i++) {
                try {
                    if (employeesBio[i].name === "Utilisateur LinkedIn") {
                        const researches = await searchInGoogle(
                            page2,
                            employeesBio[i].bio
                        );
